...
router.get('/', auth, async (req, res) => {
  try {
    const data = await InternalExpense.find({ userId: req.userId }).sort({ date: -1 });
    res.json(data);
  } catch (err) {
...
router.get('/:id', auth, async (req, res) => {
  try {
    const item = await InternalExpense.findOne({ _id: req.params.id, userId: req.userId });
...
router.delete('/:id', auth, async (req, res) => {
  try {
    const item = await InternalExpense.findOneAndDelete({ _id: req.params.id, userId: req.userId });
    if (!item) return res.status(404).json({ message: 'Data tidak ditemukan' });
